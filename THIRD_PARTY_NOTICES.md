# Third-party notices

## litehtml

- Source: https://github.com/litehtml/litehtml
- Pinned revision: `d4cbcba78e408bedc5f305459e37e5c2ceef0af3`
- License: BSD 3-Clause
- Copyright (c) 2013, Yuri Kobets (tordex)

The complete license text is available in `third_party/litehtml/LICENSE`.

## gumbo-parser

litehtml includes gumbo-parser for HTML5 parsing.

- Source: `third_party/litehtml/src/gumbo`
- License: Apache License 2.0

The complete notice and license texts are available in the gumbo source directory included by
the litehtml submodule.

## Backdrop

- Source: https://github.com/Kyant0/AndroidLiquidGlass
- Version: `2.0.0`
- License: Apache License 2.0
- Copyright (c) 2025 Kyant

The liquid bottom tabs, slider, toggle, magnifier, interaction highlight, and damped-drag
implementations include code adapted from the official catalog application. The adapted
files contain local changes for Murexide navigation, accessibility, discrete values, and
Material fallbacks.

## Kyant Shapes

- Source: https://github.com/Kyant0/Shapes
- Version: `1.2.0`
- License: Apache License 2.0
- Copyright (c) 2025 Kyant

